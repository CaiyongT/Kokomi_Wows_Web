<template>
    <div class="user-info-page">
      ...
      >
        ...
        <a-tab-pane v-for="tab in realTabs" :key="tab.key">
          <!-- 自定义标签头 -->
          <template #tab>
            <span style="margin:0px 20px;">{{ tabTitle(tab) }}</span>
            <!-- 关闭按钮（阻止事件冒泡避免触发标签切换） -->
            <CloseOutlined @click.stop="closeTab(tab.key)" />
          </template>
  
          <!-- 内容区域：根据 userId 显示不同内容 -->
          <UserSearchForm 
            v-if="tab.userId === null" 
            @onSearch="handleSearch(tab.key, $event)" 
          />
          <UserDetailTabs v-else :userId="tab.userId"  />
        </a-tab-pane>
        ...
        > 
          <template #tab>
            <a-tooltip color="blue" :title="realTabs.length >=5 ? '页签达到最大了喵' : '新增标签页'">
              <span>
                <PlusOutlined />
              </span>
            </a-tooltip>
  ...
  function handleSearch(tabKey, userId,userName) {
    const tab = realTabs.value.find(t => t.key === tabKey)
    if (tab) {
      tab.userId = userId // 更新为详情展示模式
  ...
  function tabTitle(tab) {
    return tab.userId ? tab.userId.name : '搜索' // 根据状态显示不同标题
  }
